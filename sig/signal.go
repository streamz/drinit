...
func New(opts SignalOpts) *Signalh {
    ...
    if trapf == nil {
        trapf = noop
        if len(traps) > 0 {
            log.Warn("signal handler is nil, ignoring traps")
            traps = []os.Signal{}
        }
    }
    
    if fwrdf == nil {
        log.Warn("forward handler is nil, signals will not be forwarded to children")
        fwrdf = noop
    }
...
func SignalNames() []string {
    res := make([]string, 0, len(name2signal))
    for k := range name2signal {
        res = append(res, k)
    }
    return res
}

// SignalName string to Signal
func SignalName(sig syscall.Signal) (string, error) {
    if name, ok := signal2name[sig]; ok {
		return name, nil
    }
